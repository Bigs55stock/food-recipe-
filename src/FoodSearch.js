...
function FoodSearch (){
    ...
    const [url, setUrl] =useState('https:/www.themealdb.com/api/json/process.env.API_URL/search.php?f=a')
    ...
    const setIndex=(letter) =>{
        setUrl(`https:/www.themealdb.com/api/json/process.env.API_URL/search.php?f=${letter}`)
    }

    const searchFood=(evt)=> {
        if (evt.key === "Enter"){
            setUrl(`https:/www.themealdb.com/api/json/process.env.API_URL/search.php?s=${search}`)
        }
    }
