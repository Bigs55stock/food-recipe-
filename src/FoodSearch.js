...
function FoodSearch (){
    ...
    const [url, setUrl] =useState(`https:/www.themealdb.com/api/json/v2/${process.env.APIKEYS}/search.php?f=a`)
    ...
    const setIndex=(letter) =>{
        setUrl(`https:/www.themealdb.com/api/json/v2/${process.env.APIKEYS}/search.php?f=${letter}`)
    }

    const searchFood=(evt)=> {
        if (evt.key === "Enter"){
            setUrl(`https:/www.themealdb.com/api/json/v2/${process.env.APIKEYS}/search.php?s=${search}`)
        }
    }
